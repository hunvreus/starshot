...
Full sync refreshes source metadata, fetches the complete GitHub list, marks missing active memberships inactive, and refreshes stale active profile details. Clear mode deletes local membership rows for the source without calling GitHub, so the next update rebuilds from GitHub. Profile refresh runs are also available as a separate backend mode and update stale cached GitHub user profiles with bounded concurrency controlled by `GITHUB_PROFILE_CONCURRENCY`.
